# English Exam Trainer — A1 to C2 Practice

English Exam Trainer is a single-page web app for preparing **standardized English exams from A1 to C2** — the Cambridge English ladder: A1 foundation, A2 Key/KET, B1 Preliminary/PET, B2 First/FCE, C1 Advanced/CAE, C2 Proficiency/CPE. Pick your level on the home screen and everything — practice tasks, vocabulary, scoring and examiner-style feedback — is pitched at that level, with progress kept separately per level. Every practice set is generated fresh by an AI model (Anthropic Claude or the Google Gemini free tier), so nothing ever repeats: you drill any of the exam's parts on demand, get instant marking with per-question explanations, and track your estimated score on the Cambridge English Scale. It is plain HTML/CSS/vanilla JS — no build step, no framework — and it runs on desktop or a phone in portrait, installs as a PWA, and stores all of your data only in your browser.
